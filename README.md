# Hi there, I'm E'zozbek! 👋

### 🚀 Aspiring Frontend Developer | Tech Enthusiast

I am a passionate beginner in the world of web development. Currently, I am focusing on mastering the building blocks of the web and building a solid foundation for my future career as a Frontend Developer.

---

### 🛠️ Tech Stack & Tools

* **Languages:** HTML5, CSS3
* **Version Control:** Git, GitHub
* **Editor:** VS Code
* **Operating System:** Windows/Linux (Optional: o'zingiznikini yozing)

---

### 🌱 What I'm Doing Right Now

* 🔭 Working on improving my **CSS Flexbox and Grid** skills.
* 📚 Learning the basics of **Responsive Web Design**.
* ⚡ Fun fact: I believe that a clean code is a happy code!

---

### 📈 GitHub Stats

![Ezozbek's GitHub stats](https://github-readme-stats.vercel.app/api?username=User-20122025&show_icons=true&theme=radical)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=User-20122025&layout=compact&theme=radical)

---

<details>
<summary><b>🇺🇿 O'zbek tilida batafsil ma'lumot</b></summary>

### Assalomu alaykum! 
Mening ismim **E'zozbek**. Men hozirda **Frontend dasturlash** yo'nalishida ilk qadamlarimni tashlayapman. 

**Hozirgi bilimlarim:**
* **HTML & CSS:** Web-sahifalarning tuzilishi va ko'rinishini yaratish.
* **Git & GitHub:** Loyihalarni boshqarish va versiyalar bilan ishlash.
* **VS Code:** Kod yozish uchun asosiy qurolim va uni turli extensionlar bilan optimallashtirishni o'rganyapman.

Mening maqsadim — sifatli, chiroyli va foydalanuvchilar uchun qulay interfeyslar yaratadigan professional dasturchi bo'lib yetishish. Har kuni yangi bilimlar olishga va loyihalar ustida ishlashga harakat qilyapman.

### 📈 GitHub Stats

![Ezozbek's GitHub stats](https://github-readme-stats.vercel.app/api?username=User-20122025&show_icons=true&theme=radical)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=User-20122025&layout=compact&theme=radical)

---
</details>
